...
public class JacksonJsonZipTest implements JavaSamplerClient {
	...
	private Message test() throws JsonGenerationException, JsonMappingException, IOException {
		...
		byte[] zipped = GzipUtil.compress(jsonResult.getBytes());
		String unzipeed = new String(GzipUtil.decompress(zipped));
		Message messageResult = mapper.readValue(unzipeed, Message.class);
